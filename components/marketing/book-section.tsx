        ...
        {collections.map((c, i) => (
          <Reveal key={c.id} delay={i * 80}>
            <button
              type="button"
              onClick={() => start(c.id)}
              className="group relative overflow-hidden bg-surface-container-low text-left w-full active:scale-[0.995] transition-transform duration-200 ease-[cubic-bezier(0.23,1,0.32,1)]"
            >
              <div className="aspect-[16/10] overflow-hidden">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  alt={c.name}
                  className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-[1.04] transition-[filter,transform] duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)]"
                  src={c.heroImage}
                />
              </div>
              <div className="p-8 md:p-12 liquid-glass border-t-0">
                ...
                <div className="w-full py-5 border border-white/10 text-label-caps text-white text-center group-hover:bg-white group-hover:text-obsidian transition-[background-color,color,border-color] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)]">
                  Explore Sets
                </div>
              </div>
            </button>
